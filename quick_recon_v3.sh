...
for t in "${REQUIRED_TOOLS[@]}"; do
    command -v "$t" >/dev/null 2>&1 || { echo -e "${RED}[!] Missing $t${RESET}"; }
done
...
echo -e "${GREEN}[+] $(safe_count "$BASE_DIR/subdomains.txt") subdomains found${RESET}"
...
if ! curl -s --max-time 15 "$WAYBACK_URL" | sort -u > "$WAYBACK_FILE"; then
    echo -e "${RED}[!] Wayback request failed. Skipping Wayback phase.${RESET}"
    touch "$WAYBACK_FILE" "$WAYBACK_LIVE"
else
    WB_TOTAL=$(safe_count "$WAYBACK_FILE")
    echo -e "${GREEN}[+] Wayback URLs collected: $WB_TOTAL${RESET}"

    if [ -s "$WAYBACK_FILE" ]; then

        grep -Evi "\.(jpg|jpeg|png|gif|svg|css|js|woff|woff2|ttf|ico|mp4|mp3|avi|pdf|axd)(\?|$)" "$WAYBACK_FILE" > "$WAYBACK_FILE.tmp" || true
        mv "$WAYBACK_FILE.tmp" "$WAYBACK_FILE"

        echo -e "${YELLOW}[*] Probing Wayback URLs with httpx...${RESET}"

        if httpx -l "$WAYBACK_FILE" \
            -mc 200 \
            -silent \
            -threads 100 \
            -timeout 5 \
            ...
            grep -Evi "\.(jpg|jpeg|png|gif|svg|css|js|woff|woff2|ttf|ico|mp4|mp3|avi|pdf|axd)(\?|$)" "$WAYBACK_TEMP" > "$WAYBACK_LIVE" || true
            rm -f "$WAYBACK_TEMP"

            WB_LIVE_COUNT=$(safe_count "$WAYBACK_LIVE")
            echo -e "${GREEN}[+] Clean Wayback live URLs: $WB_LIVE_COUNT${RESET}"
        else
            echo -e "${RED}[!] httpx failed on Wayback URLs. Skipping.${RESET}"
            touch "$WAYBACK_LIVE"
        fi

    else
        echo -e "${RED}[!] No Wayback URLs found.${RESET}"
        touch "$WAYBACK_LIVE"
    fi
fi
...
echo -e "${GREEN}[+] Live hosts: $(safe_count "$BASE_DIR/live_subdomains.txt")"
echo -e "${GREEN}[+] Unique IPs: $(safe_count "$BASE_DIR/unique_ips.txt")${RESET}"
...
if [ -s "$BASE_DIR/unique_ips.txt" ]; then
    ...
    touch "$BASE_DIR/nmap_results.txt"
fi

echo -e "${GREEN}[✔] Recon finished for $DOMAIN${RESET}"

# ---- Ensure all required files exist ----
ensure_file "$BASE_DIR/subdomains.txt"
ensure_file "$BASE_DIR/live_subdomains.txt"
ensure_file "$BASE_DIR/unique_ips.txt"
ensure_file "$BASE_DIR/subdomain_ip_map.csv"
ensure_file "$BASE_DIR/live_urls.txt"
ensure_file "$BASE_DIR/nmap_results.txt"

WB_TOTAL=${WB_TOTAL:-0}
WB_LIVE_COUNT=${WB_LIVE_COUNT:-0}
...
<div class="stats">
<div class="card"><h3>Subdomains</h3><p>$(safe_count "$BASE_DIR/subdomains.txt")</p></div>
<div class="card"><h3>Live Hosts</h3><p>$(safe_count "$BASE_DIR/live_subdomains.txt")</p></div>
<div class="card"><h3>IPs</h3><p>$(safe_count "$BASE_DIR/unique_ips.txt")</p></div>
